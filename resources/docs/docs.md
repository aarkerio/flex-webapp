...
The `home-routes` handler in the `flex-webapp.routes.home` namespace
...
The `home-page` function will in turn call the `flex-webapp.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/flex_webapp/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `flex-webapp.handler` namespace:
...
Request middleware functions are located under the `flex-webapp.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `flex-webapp.dev-middleware` namespace found in
